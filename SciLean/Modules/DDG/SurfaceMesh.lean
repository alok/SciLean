...
/-- Generate a random floating point number in {lit}`[0, 1]` -/
...
/-- Create a random vertex with coordinates sampled from uniform {lit}`[0, 1]` -/
...
/-- Create {name}`nvertices` random vertices with coordinates sampled from {lit}`[-scale/2, scale/2]` -/
...
/-- Create a random mesh with {name}`nfaces` random faces. Each face is generated by {name}`randTriFace` -/
...
/-- Create a random mesh with {name}`nvertices` vertices and {name}`nfaces` faces -/
