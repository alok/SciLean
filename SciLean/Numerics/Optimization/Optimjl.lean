...
An example of such limiting process would be sending {lit}`g_abstol` to zero and {lit}`iterations` to infinity.

Most definitelly there is not such universal limiting process as the convergence will surelly depend
on the particular optimizer and the function we optimize. As for now we just keep it as {lit}`opaque`
preventing us from proving anything about it. -/
opaque Options.filter {R : Type} [RealScalar R] : Filter (Options R) := default


/-- Finding minimum of a function an be approximated by running {lit}`optimize` with optimization
algorithm {lit}`method` with initial guess {lit}`x₀`.

This is most definitelly missing crucial assumptions on {lit}`f`, {lit}`method`, {lit}`x₀` to be actually true.
