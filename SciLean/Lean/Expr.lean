...
where
  ...
  | .noMatch    => match e with
    ...
    | .letE _ t v b nonDep  =>
      match ← run t with
      | .done t => return .done (e.updateLet! t v b nonDep)
      | .yield t =>
      match ← run v with
      | .done v => return .done (e.updateLet! t v b nonDep)
      | .yield v =>
      match ← run b with
      | .done b => return .done (e.updateLet! t v b nonDep)
      | .yield b => return .yield (e.updateLet! t v b nonDep)
