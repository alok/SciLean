...
/-- This axiom is obviously contradictory. We use it to compile programs that were designed only for reals or proper fields. With this axiom we can plug {name}``Float`` to function that would only accept types that have instance of {lit}``RCLike``.
